...
namespace NW_Lib
{
	...
	*/
								   ...
								   DWORD dwNumBuffers )
	{
		...
		// Create the sound
		*ppSound = new CSound( apDSBuffer, dwDSBufferSize, dwNumBuffers, pWaveFile, dwCreationFlags , &m_fVolume );
	...
	CSound::CSound( LPDIRECTSOUNDBUFFER* apDSBuffer, DWORD dwDSBufferSize, 
					DWORD dwNumBuffers, CWaveFile* pWaveFile, DWORD dwCreationFlags, FLOAT* pfVolume )
	{
		DWORD i;
	...
	//-----------------------------------------------------------------------------
	HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags, FLOAT FVolume, LONG lFrequency, LONG lPan )
	{
		HRESULT hr;
		BOOL    bRestored;
		...
		{
			pDSB->SetVolume( CulVolume((*m_pfVolume) + FVolume) );
		}
	...
	CStreamingSound::CStreamingSound( LPDIRECTSOUNDBUFFER pDSBuffer, DWORD dwDSBufferSize, 
					...
					: CSound( &pDSBuffer, dwDSBufferSize, 1, pWaveFile, 0 , &m_fVolume)           
	{
		m_dwLastPlayPos     = 0;
		m_dwPlayProgress    = 0;
